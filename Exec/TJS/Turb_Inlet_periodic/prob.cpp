...
extern "C" {
  ...
  // Compute dk based on a discretization of M different discrete wavenumbers
    ...
    PeleC::prob_parm_host->k.resize(3);
    PeleC::prob_parm_host->p.resize(3);
    PeleC::prob_parm_host->q.resize(3);
    PeleC::prob_parm_host->xi.resize(3);
    PeleC::prob_parm_host->zeta.resize(3);
    PeleC::prob_parm_host->omega.resize(PeleC::h_prob_parm_device->sampling_number);

    for (int i = 0; i < 3; i++){

      PeleC::prob_parm_host->k[i].resize(PeleC::h_prob_parm_device->sampling_number);
      PeleC::prob_parm_host->p[i].resize(PeleC::h_prob_parm_device->sampling_number);
      PeleC::prob_parm_host->q[i].resize(PeleC::h_prob_parm_device->sampling_number);
      PeleC::prob_parm_host->xi[i].resize(PeleC::h_prob_parm_device->sampling_number);
      PeleC::prob_parm_host->zeta[i].resize(PeleC::h_prob_parm_device->sampling_number);

      for (int n = 0; n < PeleC::h_prob_parm_device->sampling_number; n++){

        PeleC::prob_parm_host->k[i][n].resize(PeleC::h_prob_parm_device->turb_num_modes);
        PeleC::prob_parm_host->p[i][n].resize(PeleC::h_prob_parm_device->turb_num_modes);
        PeleC::prob_parm_host->q[i][n].resize(PeleC::h_prob_parm_device->turb_num_modes);
    ...
    for (int n = 0; n < PeleC::h_prob_parm_device->sampling_number; n++){
      ...
      PeleC::prob_parm_host->omega[n].resize(PeleC::h_prob_parm_device->turb_num_modes);
  ...
  // Loop to initialize and save arrays for zeta and xi to be used in next loop
    for (int n = 0; n < PeleC::h_prob_parm_device->sampling_number; n++) {
      ...
      for (int i = 0; i < 3; i++) {
        PeleC::prob_parm_host->xi[i][n] = amrex::RandomNormal(0.0, 1.0);
        PeleC::prob_parm_host->zeta[i][n] = amrex::RandomNormal(0.0, 1.0);
      }
    }
  ...
  // Loop to initialize and save arrays for p, q, k, omega
    ...
    // Generate E for this wavenumber
      ...
      // Convert to cartesian coords
        PeleC::prob_parm_host->k[0][n][m] = k_mag * cos(theta) * sin(phi);
        PeleC::prob_parm_host->k[1][n][m] = k_mag * sin(theta) * sin(phi);
        PeleC::prob_parm_host->k[2][n][m] = k_mag * cos(phi);

      // Generate random values for getting p and q
        PeleC::prob_parm_host->omega[n][m] = amrex::RandomNormal(0.0, omega_upper_bound);
        randa = amrex::Random();

      // Calculate p, q

        xi_cross_k[0] = (PeleC::prob_parm_host->xi[1][n] * PeleC::prob_parm_host->k[2][n][m]
         - PeleC::prob_parm_host->k[1][n][m] * PeleC::prob_parm_host->xi[2][n]);
        xi_cross_k[1] = (PeleC::prob_parm_host->xi[2][n] * PeleC::prob_parm_host->k[0][n][m]
         - PeleC::prob_parm_host->k[2][n][m] * PeleC::prob_parm_host->xi[0][n]);
        xi_cross_k[2] = (PeleC::prob_parm_host->xi[0][n] * PeleC::prob_parm_host->k[1][n][m]
         - PeleC::prob_parm_host->k[0][n][m] * PeleC::prob_parm_host->xi[1][n]);
        ...
        for (int i = 0; i < 3; i++) {
          PeleC::prob_parm_host->p[i][n][m] = (xi_cross_k[i] / cross_mag) * std::sqrt(root_arg);
        }


        zeta_cross_k[0] = (PeleC::prob_parm_host->zeta[1][n] * PeleC::prob_parm_host->k[2][n][m]
         - PeleC::prob_parm_host->k[1][n][m] * PeleC::prob_parm_host->zeta[2][n]);
        zeta_cross_k[1] = (PeleC::prob_parm_host->zeta[2][n] * PeleC::prob_parm_host->k[0][n][m]
         - PeleC::prob_parm_host->k[2][n][m] * PeleC::prob_parm_host->zeta[0][n]);
        zeta_cross_k[2] = (PeleC::prob_parm_host->zeta[0][n] * PeleC::prob_parm_host->k[1][n][m]
         - PeleC::prob_parm_host->k[0][n][m] * PeleC::prob_parm_host->zeta[1][n]);
      ...
      // amrex::Print() << "Root Arg = " << root_arg << '\n' << "Random# = " << randa << '\n' << "E = " << E << '\n';
        for (int i = 0; i < 3; i++) {
          PeleC::prob_parm_host->q[i][n][m] = (zeta_cross_k[i] / cross_mag) * std::sqrt(root_arg);
        }

      // Normalize k to k_tilde using the lowest wave number (equation 21 Huang 2010)
        for (int i = 0; i < 3; i++) {
         PeleC::prob_parm_host->k[i][n][m] /= k_lo;
...
{
 if (amrex::ParallelDescriptor::IOProcessor()) {
   amrex::Print() << "TIME= " << PeleC::h_prob_parm_device->timechk << " p[0]  = " << PeleC::prob_parm_host->p[0][5][10] << '\n';
   amrex::Print() << "TIME= " << PeleC::h_prob_parm_device->timechk << " q[0]  = " << PeleC::prob_parm_host->q[0][5][10] << '\n';
   amrex::Print() << "TIME= " << PeleC::h_prob_parm_device->timechk << " k[0]  = " << PeleC::prob_parm_host->k[0][5][10] << '\n';
   amrex::Print() << "TIME= " << PeleC::h_prob_parm_device->timechk << " k[1]  = " << PeleC::prob_parm_host->k[1][5][10] << '\n';
   amrex::Print() << "TIME= " << PeleC::h_prob_parm_device->timechk << " omega  = " << PeleC::prob_parm_host->omega[5][10] << '\n';
